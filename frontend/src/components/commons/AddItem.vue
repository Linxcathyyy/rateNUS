    ...
    <ValidationObserver ref="addItemObserver">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="warning" depressed @click="save" :loading="loading"
            >Save</v-btn
          >
        </v-card-actions>
      </v-card>
  ...
  methods: {
    ...
    async addItemInDB(data) {
      ...
      switch (this.type) {
        ...
        case "Stall":
          ...
          break;
        case "Study Area":
          result = await StudyAreaRequest.addStudyArea(
            jwtToken,
            username,
            data
          );
    ...
    async save() {
      ...
      if (isValidated) {
        ...
        const facilities = this.item.facilities
          ? this.item.facilities.toString().split(",")
          : [];
        const data = {
          name: this.item.name,
          ...
          ...(this.item.lowestPrice && {
            lowestPrice: Number(this.item.lowestPrice),
          }),
          ...(this.item.highestPrice && {
            highestPrice: Number(this.item.highestPrice),
          }),
        };
        ...
        await this.addItemInDB(data)
          .then(async (response) => {
            if (response === 200) {
              ...
              // console.log("fail");
              this.$refs.saveError.innerHTML =
                "Failed to add this item, please try again";
            }
          })
          .catch(() => {
            // console.log("fail");
